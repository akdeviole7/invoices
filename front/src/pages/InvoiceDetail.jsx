...
function InvoiceDetail() {
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      // Get the API base URL (adjust this to match your backend URL)
      const API_BASE_URL = import.meta.env.VITE_API_URL || 'http://localhost:5001';
      
      // Fetch the PDF
      const response = await fetch(`${API_BASE_URL}/api/pdf/invoice/${id}`, {
        method: 'GET',
        headers: {
      ...
      <div className="bg-white p-6 rounded-xl shadow-md mb-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 print:hidden">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button 
            ...
            className="bg-white text-indigo-600 px-4 py-2 rounded-lg border-2 border-indigo-600 hover:bg-indigo-600 hover:text-white transition-all duration-200 flex items-center gap-2 text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <Download size={18} />
